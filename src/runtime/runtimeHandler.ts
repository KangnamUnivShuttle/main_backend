...
export function returnRecommendedMessage(
  recommendNextBlockList: NextBlockModel[]
): KakaoChatResModel {
  const returnMsg = {
    version: "2.0",
    template: {
      ...
      quickReplies: [] as QuickReplyModel[],
    },
  };
...
export async function getRecentUserState(
  userkey: string
): Promise<{ blockID: string; inputMsg: string | null }> {
  ...
  try {
    ...
    if (chatUser) {
      logger.debug(
        `[runtimeHandler] [getRecentUserState] userkey: ${userkey}, last block: ${chatUser.currentBlockId}`
      );
    ...
    logger.error(
      `[runtimeHandler] [getRecentUserState] userkey: ${userkey} error: ${err.message}`
    );
    return Promise.resolve({
      blockID: lastBlockId,
      inputMsg: null,
...
export async function writeFallbackEscapeLog(
  userKey: string,
  selectedRecommendedBlockId: string
) {
  const connection = getConnection();
  ...
  try {
    ...
    logger.debug(
      `[runtimeHandler] [writeFallbackEscapeLog] Fallback escape log for ${userKey}`
    );
    ...
    if (!fallback) {
      logger.error(
        `[runtimeHandler] [writeFallbackEscapeLog] User ${userKey}, is not in fallback`
      );
      throw new Error(`User ${userKey}, is not in fallback`);
    }

    logger.debug(
      `[runtimeHandler] [writeFallbackEscapeLog] Fallback id: ${fallback.fallbackId}`
    );
    ...
    logger.debug(
      `[runtimeHandler] [writeFallbackEscapeLog] transaction committed`
    );
  } catch (err: any) {
    logger.error(
      `[runtimeHandler] [writeFallbackEscapeLog] transaction rollback: ${err.message}`
    );
...
export async function openFallbackBlock(
  userKey: string,
  cameBlockID: string,
  recommendNextBlockList: NextBlockModel[],
  messageText: string
  ...
  try {
    ...
    logger.debug(
      `[runtimeHandler] [openFallbackBlock] fallback idx: ${fallbackID}`
    );
    ...
    await queryBuilder
      ...
      .values(
        recommendNextBlockList.map((item) => {
          logger.debug(
            `[runtimeHandler] [openFallbackBlock] fallback id: ${fallbackID} / block link id: ${item.blockLinkedID}`
          );
          return {
    ...
    logger.info(
      `[runtimeHandler] [openFallbackBlock] userkey: ${userKey} fallback idx updated: ${fallbackID}`
    );

    await queryRunner.commitTransaction();

    logger.debug(
      `[runtimeHandler] [openFallbackBlock] userkey: ${userKey} / ${cameBlockID} transaction committed`
    );
  } catch (err: any) {
    logger.error(
      `[runtimeHandler] [openFallbackBlock] userkey: ${userKey} / ${cameBlockID} transaction rollback: ${err.message}`
    );
    await queryRunner.rollbackTransaction();
  } finally {
    logger.info(
      `[runtimeHandler] [openFallbackBlock] userkey: ${userKey} / ${cameBlockID} transaction done.`
    );
    await queryRunner.release();
  }
}
...
export async function updateUserState(
  ...
  logger.debug(
    `[runtimeHandler] [updateUserState] userkey: ${userkey}, set blockID: ${blockID}`
  );
  ...
  try {
    logger.debug(
      `[runtimeHandler] [updateUserState] userkey: ${userkey} start transaction`
    );
    ...
    if (isExistUser) {
      logger.debug(
        `[runtimeHandler] [updateUserState] userkey: ${userkey} already exist`
      );
      await queryBuilder
        .update()
        .set({
      ...
      logger.debug(
        `[runtimeHandler] [updateUserState] userkey: ${userkey} newbie`
      );
      await queryBuilder
        .insert()
        .into(ChatUser)
        .values([{ userkey, currentBlockId: blockID }])
        .execute();
    }
    ...
    logger.debug(
      `[runtimeHandler] [updateUserState] userkey: ${userkey} transaction committed`
    );
  } catch (err: any) {
    logger.error(
      `[runtimeHandler] [updateUserState] userkey: ${userkey} transaction rollback: ${err.message}`
    );
    await queryRunner.rollbackTransaction();
  } finally {
    logger.info(
      `[runtimeHandler] [updateUserState] userkey: ${userkey} transaction done.`
    );
    await queryRunner.release();
  }
}
